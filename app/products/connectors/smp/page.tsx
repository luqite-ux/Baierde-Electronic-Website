...
export default async function SMPSeriesPage() {
  const smpProducts = await getConnectorProducts({ series: ["SMP"] })
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-12">
        ...
        {smpProducts.length > 0 ? (
              ...
              >
                <Card className="h-full hover:shadow-lg transition-shadow">
                  <CardContent className="p-4">
                    <Image
                      src={product.imageUrl || "/placeholder.svg?height=300&width=300&query=SMP+RF+connector"}
                      alt={product.title}
                      width={300}
                      height={300}
          ...
          <p className="text-muted-foreground">暂无 SMP 系列产品，请稍后在 Sanity 中添加。</p>
        )}
      </div>
